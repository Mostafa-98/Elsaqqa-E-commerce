      ...
      <aside id="logo-sidebar" className="fixed top-0 left-0  w-64 h-screen transition-transform -translate-x-full sm:translate-x-0" aria-label="Sidebar">
        <div className="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
          <Link to={"/"} className="flex items-center ps-2.5 mb-5">
            ...
            <span className="self-center text-xl font-semibold whitespace-nowrap dark:text-white">ELSAQA</span>
          </Link>
          <ul className="space-y-2 font-medium">
            ...
            <li>
              <Link to={"/cart"} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                ...
                <span className="inline-flex items-center justify-center w-3 h-3 p-3 ms-3 text-sm font-medium text-blue-800 bg-blue-100 rounded-full dark:bg-blue-900 dark:text-blue-300">{numOfItems}</span>
              </Link>
            </li>
            <li>
              <Link to={"/category"} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                ...
                <span className="flex-1 ms-3 whitespace-nowrap">Category</span>              </Link>
            </li>
            <li>
            ...
            {token ? <li>
              <Link onClick={logout}  to={""} className="flex items-center p-2 text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 group">
                <svg className="shrink-0 w-5 h-5 text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700">
          <div className="grid grid-cols-1 gap-4 mb-4">
            {data?.data.map(function (order, idx) {
              return <div key={idx} className="flex flex-col items-center justify-center rounded-sm bg-gray-50 dark:bg-gray-800">
                <h2 className="text-4xl font-bold text-[#0B50B9]">Total Order Price : {order.totalOrderPrice} EGP </h2>
                <h2 className="text-2xl font-bold text-[#0B50B9]"> Payment Method Type :  {order.paymentMethodType}</h2>
